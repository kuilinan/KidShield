...
public class GuardService extends Service {
    ...
    /**
     * AlarmReceiver：当 GuardService 被杀死时，通过 Alarm 重新拉起
     */
    public static class AlarmSelfHealReceiver extends BroadcastReceiver {
        @Override
        public void onReceive(Context context, Intent intent) {
            if (!AlarmSelfHealReceiver.isServiceRunning(context)) {
        ...
        private static boolean isServiceRunning(Context context) {
            try {
                ...
                return (System.currentTimeMillis() - lastHeartbeat) < 30000; // 30秒内有心跳则视为存活
            } catch (Exception e) {
                return false;
            }
        }
